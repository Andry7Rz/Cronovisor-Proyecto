...
function RelojFlotante({ epocas = [], epocaActual, alCambiarEpoca }) {
  const [expandido, setExpandido] = React.useState(false);
  const [arrastrando, setArrastrando] = React.useState(false);
  const [anguloArrastre, setAnguloArrastre] = React.useState(null);
  
  // Estado local para garantizar que la interfaz visual NUNCA regrese al botón 1 por error
  const [indiceSeleccionado, setIndiceSeleccionado] = React.useState(0);
  
  const svgRef = React.useRef(null);

  // Ángulos exactos: Época 1 (9:00 = 270°), Época 2 (12:00 = 0°), Época 3 (3:00 = 90°)
  const angulosEpocas = [270, 0, 90];

  // Sincronizar el estado local si epocaActual cambia desde el exterior
  React.useEffect(() => {
    if (!epocas || epocas.length === 0 || !epocaActual) return;

    const idx = epocas.findIndex((e, i) => {
      if (e === epocaActual) return true;
      
      const actId = typeof epocaActual === 'object' ? epocaActual.id : epocaActual;
      const actNom = typeof epocaActual === 'object' ? epocaActual.nombre : epocaActual;

      if (e.id !== undefined && actId !== undefined && String(e.id) === String(actId)) return true;
      if (e.nombre && actNom && String(e.nombre).toLowerCase() === String(actNom).toLowerCase()) return true;
      if (typeof epocaActual === 'number' && epocaActual === i) return true;
  ...
  // Función unificada para cambiar época (actualiza la vista inmediatamente)
  const cambiarAEpoca = (index) => {
    if (index < 0 || index >= epocas.length) return;
    setIndiceSeleccionado(index);
    if (alCambiarEpoca && epocas[index]) {
      alCambiarEpoca(epocas[index]);
    }
  };

  const anguloBase = angulosEpocas[indiceSeleccionado % angulosEpocas.length];

  // Cálculo de época según el ángulo del cursor al arrastrar
  const calcularIndicePorAngulo = (deg) => {
    let a = deg % 360;
    if (a < 0) a += 360;

    const dist270 = Math.abs(a - 270);
    const dist0 = Math.min(a, 360 - a);
    const dist90 = Math.abs(a - 90);

    if (dist270 <= dist0 && dist270 <= dist90) return 0; // Prehispánica
    if (dist0 <= dist270 && dist0 <= dist90) return 1;    // Colonial
    return 2;                                             // Actual
  };

  const obtenerAnguloDesdeEvento = (e) => {
  ...
  const iniciarArrastre = (e) => {
    setArrastrando(true);
    const deg = obtenerAnguloDesdeEvento(e);
    const idx = calcularIndicePorAngulo(deg);
    setAnguloArrastre(deg);
    cambiarAEpoca(idx);
  };
  ...
  const finalizarArrastre = () => {
    if (arrastrando) {
      ...
      setAnguloArrastre(null);
    }
  };

  const anguloVisual = arrastrando && anguloArrastre !== null ? anguloArrastre : anguloBase;

  return (
    <div className="reloj-flotante">
      {/* Botón Flotante Cerrado */}
      <button 
        className="icono-reloj-minimalista" 
        onClick={() => setExpandido(!expandido)}
        title="Cambiar época"
      >
        <svg viewBox="0 0 100 100" className="svg-reloj-mini">
          <circle cx="50" cy="50" r="40" className="esfera-mini" />
          <line x1="50" y1="14" x2="50" y2="24" className="marca-mini" />
          <line x1="86" y1="50" x2="76" y2="50" className="marca-mini" />
          <line x1="14" y1="50" x2="24" y2="50" className="marca-mini" />
          
          <line 
            x1="50" y1="50" x2="50" y2="24" 
            className="aguja-mini" 
            style={{ transform: `rotate(${anguloBase}deg)` }}
          />
          <circle cx="50" cy="50" r="4" className="centro-mini" />
        </svg>
      </button>

      {/* Panel Desplegado */}
      {expandido && (
        <div className="panel-reloj-minimalista">
          <div className="encabezado-reloj">
            <span>SELECTOR TEMPORAL</span>
            <button className="btn-cerrar-reloj" onClick={() => setExpandido(false)}>✕</button>
          </div>

          <div className="contenedor-esfera">
            <svg 
              ref={svgRef}
              viewBox="0 0 200 200" 
              className="svg-reloj-grande"
              onPointerDown={iniciarArrastre}
              onPointerMove={manejarMover}
            ...
            >
              ...
              <circle cx="100" cy="100" r="88" className="esfera-fondo" />
              <circle cx="100" cy="100" r="88" className="esfera-borde-grueso" />

              {/* Marcas Horarias Gruesas */}
              {[...Array(12)].map((_, i) => (
                <line
                  key={i}
                  x1="100" y1="20" x2="100" y2={i % 3 === 0 ? "28" : "24"}
                  transform={`rotate(${i * 30} 100 100)`}
                  className={i % 3 === 0 ? "marca-principal" : "marca-secundaria"}
                />
              ))}

              {/* Nodos Interactivos (1, 2, 3) */}
              {epocas.map((epoca, index) => {
                const angulo = angulosEpocas[index % angulosEpocas.length];
                const esActiva = index === indiceSeleccionado;
                ...
                return (
                  <g 
                    key={epoca.id || index} 
                    className={`nodo-epoca ${esActiva ? 'activo' : ''}`}
                    onClick={(e) => {
                      e.stopPropagation();
                      cambiarAEpoca(index);
                    }}
                  >
                    <circle cx={cx} cy={cy} r="14" className="circulo-nodo" />
                    <text x={cx} y={cy + 4.5} textAnchor="middle" className="texto-nodo">
                      {index + 1}
                    </text>
                  </g>
                );
              ...
              {/* AGUJA PRINCIPAL */}
              <g 
                className="grupo-aguja"
                style={{ 
                  transform: `rotate(${anguloVisual}deg)`,
                  transition: arrastrando ? 'none' : 'transform 0.45s cubic-bezier(0.34, 1.56, 0.64, 1)'
                }}
              >
                <line x1="100" y1="100" x2="100" y2="114" className="aguja-contrapeso" />
                <line x1="100" y1="100" x2="100" y2="28" className="aguja-principal-gruesa" />
                <circle cx="100" cy="28" r="5" className="aguja-punta-gruesa" />
              </g>
              
              {/* Centro de Rotación */}
              <circle cx="100" cy="100" r="6" className="centro-aguja" />
              <circle cx="100" cy="100" r="2" className="centro-aguja-punto" />
            </svg>
          </div>

          {/* Nombre de la Época Activa */}
          <div className="info-epoca-minimalista">
            <span className="etiqueta-epoca">Época Seleccionada</span>
            <h4 className="titulo-epoca">
              {epocas[indiceSeleccionado] ? epocas[indiceSeleccionado].nombre : '---'}
            </h4>
          </div>

          {/* Lista de Botones Inferiores */}
          <div className="lista-botones-epocas">
            {epocas.map((epoca, index) => {
              const esActiva = index === indiceSeleccionado;
              return (
                <button
                  key={epoca.id || index}
                  className={`btn-epoca-item ${esActiva ? 'activo' : ''}`}
                  onClick={() => cambiarAEpoca(index)}
                >
                  <span className="num-epoca">{index + 1}</span>
                  <span className="nombre-epoca">{epoca.nombre}</span>
                </button>
              );
            })}
          </div>
...
function Mapa({ epocaActual }) {
  const [tooltipInfo, setTooltipInfo] = useState({ texto: '', x: 0, y: 0, visible: false });

  const manejarMouseMove = (e, nombre) => {
    setTooltipInfo({
      texto: nombre,
      x: e.clientX + 15,
      y: e.clientY - 35,
...
function Finale({epocaActual}) {
  ...
  <div className="seccion-contenido">
    <h2>Conclusion</h2>
    <p>prometo que no fue mas que un malentendido, ya sabes que con el alchol todo lo olvido. no se su nombre y mucho
      menos su apellido, y andan diciendo que lo que hice fue indevido. es mi palaabra contra la de ellos.
      que talvez inventen porque nos tienen celos. que muestren pruebas de aquellos hechos
      aunque estaba borracho asi no lo recuerdo. porfa no dudes de mi yo te quiero solo a ti 
      seria incapaz de hacerte daño y no te haria sufrir. por fa no dudes de mi, yo te quiero solo a ti. es mi palabra
      contra la de ellos y tienes que elegir.
    </p>
  </div>
  )
}
